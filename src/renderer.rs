...
pub fn render_scene(scene: &Scene, canvas: &mut [u8]) -> Result<(), String> {
    ...
    let mut chunks = canvas.chunks_mut(CHUNK_SIZE_BYTES);

    // Need to know the xy position in order to draw it
    let (mut x, mut y): (usize, usize) = (0, 0);
    for (i, chunk) in chunks.enumerate() {
        let bytes_chunk = i * CHUNK_SIZE_BYTES;
        x = bytes_chunk % (super::SCREEN_WIDTH * BYTES_PIXEL); // remainder
        y = bytes_chunk / (super::SCREEN_WIDTH * BYTES_PIXEL);
        println!("Chunk[{}]: x:{}, y:{}", i, x, y);
        threads.push(thread::spawn(|| render_to_canvas(scene, chunk, (x, y))));
    }

    //render_to_canvas(&scene, x.)?;
